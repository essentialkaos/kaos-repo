...
Source0:            http://dl.4players.de/ts/releases/%{version}/%{name}-server_linux-amd64-%{version}.tar.gz
Source1:            %{name}.init
...
%prep
%setup -q -n %{name}-server_linux-amd64
...
cp ts3server_linux_amd64 %{buildroot}%{_opt}/%{name}/ts3server
